...
/* ─── Coupons (single-use discount codes) ─────────── */
const KEY_COUPON = (code) => `coupon:${String(code||'').toUpperCase()}`;
const KEY_COUPON_LIST = 'coupons:list';

export async function createCoupon({ code, amount, note }) {
  const c = String(code || '').toUpperCase().replace(/[^A-Z0-9_-]/g,'').slice(0,32);
  if (!c) throw new Error('รหัสไม่ถูกต้อง');
  const exists = await command('GET', KEY_COUPON(c));
  if (exists) throw new Error('รหัสนี้มีอยู่แล้ว');
  const coupon = {
    code: c,
  ...
  await command('SET', KEY_COUPON(c), JSON.stringify(coupon));
  await command('LPUSH', KEY_COUPON_LIST, c);
  return coupon;
}

export async function getCoupon(code) {
  const raw = await command('GET', KEY_COUPON(code));
  if (!raw) return null;
  try { return typeof raw==='string'?JSON.parse(raw):raw; } catch { return null; }
}
...
export async function redeemCoupon(code, usedBy) {
  ...
  if (!c) throw new Error('ไม่พบรหัสส่วนลดนี้');
  if (c.used) throw new Error('รหัสส่วนลดนี้ถูกใช้ไปแล้ว');
  c.used = true; c.usedBy = String(usedBy||''); c.usedAt = new Date().toISOString();
  await command('SET', KEY_COUPON(c.code), JSON.stringify(c));
  return c;
}

export async function listCoupons(limit=100) {
  const codes = await command('LRANGE', KEY_COUPON_LIST, 0, Math.max(1, Math.min(500,limit))-1) || [];
  const out = [];
  for (const c of codes) {
    const item = await getCoupon(c);
...
export async function deleteCoupon(code) {
  ...
  await command('DEL', KEY_COUPON(c));
  await command('LREM', KEY_COUPON_LIST, 0, c);
  return true;
}
